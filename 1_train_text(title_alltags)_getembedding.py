...
df = pd.read_csv(
    'train/captioning_done_geo_category_none_old_train_processed_na_time.csv')
print('read!')


selected_column = df["Title"].astype(str).tolist()
embeddings = embedder.encode(selected_column)
print(embeddings)
print(len(embeddings[0]))
embeddings = np.array(embeddings).reshape(-1, 1)
result_df = pd.DataFrame(embeddings, columns=["Title"])
result_df.to_pickle("train/newtrain_Title_multilang_embedding.pkl")
print('title')


selected_column = df["Alltags"].astype(str).tolist()
embeddings = model.encode(selected_column)
print(embeddings)
print(len(embeddings[0]))
embeddings = np.array(embeddings).reshape(-1, 1)
result_df = pd.DataFrame(embeddings, columns=["Alltags"])
result_df.to_pickle("train/newtrain_Alltag_multilang_embedding.pkl")
print('alltag')
...
df_Spatial_Temperal.to_pickle('train/newtrain_spatial_temperal.pkl')

'''
df_label = df[['Uid', 'Pid', 'label']]
df_label.to_pickle('train/newtrain_label.pkl')
'''
df_category = df[['Uid', 'Pid', 'Category', 'Subcategory', 'Concept', 'Mediatype', 'Mediastatus',
'canbuypro', 'ispro', 'Ispublic', 'City', 'State', 'Country']]
df_category.to_pickle("train/newtrain_category.pkl")
...
df_user_info.to_pickle('train/newtrain_user_info.pkl')